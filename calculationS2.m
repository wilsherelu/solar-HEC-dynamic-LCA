...
Natcap=newsolar;
NatAvg=zeros(size(Cprediction,3),1);
NSghg=newsolar;
avgsolar=newsolar;
...
Natcap(:,1)=sum(Cprediction(:,list0,1),2);
NatAvg(1)=sum(Eghg(:,1).*squeeze(Natcap(:,1)),1)./sum(squeeze(Natcap(:,1)),1);
for i=2:size(Cprediction,3)%size(Cprediction,3)

    NSghg(:,i)=(SolarGHGM+SolarE*NatAvg(i-1))./(lifespan*365*Solardaily);
    if i<=20
    ...
    Natcap(:,i)=sum(Cprediction(:,list0,i),2);
NatAvg(i)=sum(Eghg(:,i).*squeeze(Natcap(:,i)),1)./sum(squeeze(Natcap(:,i)),1);
end
natavgsolar=natavgsolar';
% xlswrite('C:\Users\lu\Desktop\job\ECfuture\avg.xlsx',NatAvg,'NatAvg','A1');
% xlswrite('C:\Users\lu\Desktop\job\ECfuture\avg.xlsx',avgsolar,'SolarAvg','A1');
% xlswrite('C:\Users\lu\Desktop\job\ECfuture\avg.xlsx',NSghg,'SolarNew','A1');
% xlswrite('C:\Users\lu\Desktop\job\ECfuture\avg.xlsx',Eghg,'State','A1');